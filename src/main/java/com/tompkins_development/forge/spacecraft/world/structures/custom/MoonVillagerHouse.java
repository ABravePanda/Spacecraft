package com.tompkins_development.forge.spacecraft.world.structures.custom;
...
public class MoonVillagerHouse extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final Codec<MoonVillagerHouse> CODEC = RecordCodecBuilder.<MoonVillagerHouse>mapCodec(instance ->
            instance.group(MoonVillagerHouse.settingsCodec(instance),
            ...
            ).apply(instance, MoonVillagerHouse::new)).codec();
    ...
    public MoonVillagerHouse(Structure.StructureSettings config,
    ...
    public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
        ...
        if (!MoonVillagerHouse.extraSpawningChecks(context)) {
    ...
    public StructureType<?> type() {
        return ModStructures.MOON_VILLAGER_HOUSE.get(); // Helps the game know how to turn this structure back to json to save to chunks
    }
}